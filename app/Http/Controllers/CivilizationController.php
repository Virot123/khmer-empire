...
class CivilizationController extends Controller
{
    public function index(Request $request){
        ...
        return view('civilizations.index',compact(['items','dateFilter','search']));
    }
        ...
        <div class="vr-popup-add-new" style="width:1000px;">
            <div class="card">
                <div class="vr-header-popup">
                    <h5 class="text-center">Add Civilization</h5>
                    <span class="vr_remove_popup vr-remove-popup"><i class="fas fa-window-close"></i></span>
                </div>
                <form class="form-card form_submit" action="/dashboard/civilization" method="post" enctype="multipart/form-data">
                '.csrf_field().'
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasfile('thumbnail'))
        {
            try {
                ...
                $file->move(public_path('storage/civilizations'), $filename);
              } catch (\Exception $e) {
                  return $e->getMessage();
              }
        }

        $item = Civilization::create([
            'thumbnail'     =>$filename,
    ...
    {
        $item   = Civilization::findOrFail($id);
        ...
        <div class="vr-popup-add-new" style="width:1000px;" >
            <div class="card">
                ...
                <form class="form-card form_submit" action="'.route('civilization.update',$id).'" method="POST" enctype="multipart/form-data">
                '.csrf_field().'
    ...
    public function update(Request $request, $id)
    {
        ...
        Civilization::where('id',$id)->update([
            'thumbnail'     =>$filename,
            'description'   =>$request->description,
            'title'         =>$request->title,
        ]);
    ...
    public function show($id)
    {
        ...
        <div class="vr-popup-view" style="width:1000px;">
            <div class="card">
                <div class="vr-header-popup">
                    <h5 class="text-center">View Civilization #'.$item->id.'</h5>
                    <span class="vr_remove_popup vr-remove-popup"><i class="fas fa-window-close"></i></span>
                </div>
                <div class="vr-body-popup">
                    <div>
                        ...
                        <div class="vr-body-image-popup">
                            <img src="'. getPhoto("civilizations",$item->thumbnail) .'" alt="">
                        </div>
                    </div>
                     <div>
